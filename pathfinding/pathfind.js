...
document.querySelectorAll(".draggable").forEach((element) => {
  dragElement(element);
  element.style.left =
    ((window.innerWidth - element.offsetWidth) / window.innerWidth) * 100 -
    1 +
    "%";
...
document
  .querySelectorAll(".AStar")
  .forEach((element) => (element.onclick = ChangeAlgorithm));
document
  .querySelectorAll(".Djikstra")
  .forEach((element) => (element.onclick = ChangeAlgorithm));
document
  .querySelectorAll(".JPS")
  .forEach((element) => (element.onclick = ChangeAlgorithm));

document
  .querySelector("#cellSizeIncrease")
  .addEventListener("click", function () {
    SetCellSize(PathfindingCookies.GetCellSize() + 1);
  });
document
  .querySelector("#cellSizeDecrease")
  .addEventListener("click", function () {
    SetCellSize(PathfindingCookies.GetCellSize() - 1);
  });
document
  ...
  .addEventListener("change", function () {
    SetCellSize(Number(this.value));
  });
document.querySelector("#cellSizeInput").value =
  PathfindingCookies.GetCellSize();

CheckFirstVisit("pathVisited");

document.querySelector("#AnimSpeed").addEventListener("click", function () {
  animationController.SetSpeed(
    animationController.speeds[
      (animationController.speeds.indexOf(animationController.speed) + 1) %
        animationController.speeds.length
    ]
  );
  this.innerHTML = `${animationController.speed}x`;
});

pageAlgorithm.changeAlgo(
  new URLSearchParams(window.location.search).get("func") || "astar"
);
...
document.querySelector("#generate").addEventListener("click", async () => {
  ...
  try {
    animationController.timeline = FindPath(
...
document.querySelector("#reset").addEventListener("click", () => {
  if (animationController.IsInProgress()) {
    Alert(
      alertContainer,
      "Animation still in progress. Something went wrong.",
      "danger"
    );
  }
  ClearAnimation();
...
document
  ...
  .addEventListener(
    "ontouchstart" in document.documentElement === true
      ? "touchstart"
      : "mousedown",
    CellHandler.bind(canvas),
    { passive: false }
  );

document
  .querySelector("#Progress-Bar-Outline")
...
function SetCellSize(newSize) {
  if (newSize < 10) {
    Alert(
      alertContainer,
      "Sizes less than 10 pixels will negatively affect performance",
      "warning"
    );
    newSize = 10;
  }
...
function SetPauseButton() {
  document
    ...
    .firstChild.setAttribute("src", "../Assets/pause-fill.svg");
}

function SetPlayButton() {
  document
    .querySelector("#PlayPause")
    .firstChild.setAttribute("src", "../Assets/play-fill.svg");
}
